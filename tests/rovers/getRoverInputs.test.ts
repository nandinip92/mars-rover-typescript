import { getRoverInitialPosition } from "../../src/rovers/getRoverInputs";
...
describe("getRoverInputs", () => {
  test("throws an error if invalid input is passed", () => {
    ...
    expect(() => {
      getRoverInitialPosition(boundary, "12345");
    }).toThrow(
      "Invalid input. Input must contain X and Y co-ordinates and a compass direction (N|E|S|W) seperated by space - X Y Direction"
    );

    expect(() => {
      getRoverInitialPosition(boundary, "1 3 5 2 7");
    }).toThrow(
      "Invalid input. Input must contain X and Y co-ordinates and a compass direction (N|E|S|W) seperated by space - X Y Direction"
    );
    expect(() => {
      getRoverInitialPosition(boundary, " 5 T N");
    }).toThrow(
      "Invalid input. Input must contain X and Y co-ordinates and a compass direction (N|E|S|W) seperated by space - X Y Direction"
    );

    expect(() => {
      getRoverInitialPosition(boundary, " 3 4 Y");
